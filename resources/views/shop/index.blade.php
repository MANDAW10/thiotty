        ...
        <div class="container-custom">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
                <div class="space-y-4">
                    ...
                    <h1 class="text-5xl md:text-7xl font-black text-slate-900 leading-[0.9] tracking-tighter">
                        {{ explode(' ', __('messages.shop'))[0] }} <span class="text-primary italic">{{ count(explode(' ', __('messages.shop'))) > 1 ? explode(' ', __('messages.shop'))[1] : '' }}</span>.
                    </h1>
                </div>
                ...
                <div class="flex items-center gap-4 w-full md:w-auto">
                    ...
                    <button @click="showFilters = true" class="md:hidden flex-1 flex items-center justify-center gap-3 py-4 bg-slate-50 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 border border-slate-100">
                        <i class="fas fa-sliders-h"></i> {{ __('messages.apply_filter') }}
                    </button>
                    ...
                    @if(isset($query) && $query)
                        <div class="hidden md:flex items-center gap-4 bg-primary/5 px-6 py-4 rounded-2xl border border-primary/10">
                             <p class="text-[10px] font-black text-primary uppercase tracking-widest">{{ __('messages.filter_results') }} : <b>"{{ $query }}"</b></p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
                ...
                <aside class="hidden lg:block lg:col-span-3 space-y-12 h-fit sticky top-32">
                    <!-- Categories -->
                    <div class="space-y-6">
                        <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] pl-4 border-l-4 border-primary">{{ __('messages.collections') }}</h3>
                        <div class="space-y-2">
                             <a href="{{ route('shop.index') }}" 
                                ...
                                <span class="text-[11px] font-black uppercase tracking-widest">{{ __('messages.view_all') }}</span>
                            ...
                            @foreach($categories as $cat)
                                <a href="{{ route('shop.index', ['category' => $cat->slug] + request()->except('category', 'page')) }}" 
                                   class="flex items-center justify-between group p-3 px-4 rounded-xl transition-all {{ request('category') == $cat->slug ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-500 hover:bg-slate-50' }}">
                                    <span class="text-[11px] font-black uppercase tracking-widest truncate">{{ $cat->display_name }}</span>
                    ...
                    <div class="space-y-6 pt-10 border-t border-slate-50">
                        <h3 class="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] pl-4 border-l-4 border-primary">{{ __('messages.budget') }}</h3>
                    ...
                    <div class="bg-primary shadow-2xl shadow-primary/30 rounded-[40px] p-8 text-white relative overflow-hidden group mt-20">
                        <div class="relative z-10 text-center">
                            ...
                            <h4 class="text-xl font-black mb-4 leading-tight">{{ __('messages.need_help') }}</h4>
                            <p class="text-[10px] font-bold text-white/70 mb-8 uppercase tracking-widest leading-relaxed">{{ __('messages.assistant_greeting') }}</p>
                ...
                <main class="lg:col-span-9 animate-fade-in-up">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-12 px-2 gap-4">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                            {{ __('messages.showing') }} {{ $products->firstItem() ?? 0 }}-{{ $products->lastItem() ?? 0 }} {{ __('messages.of') }} {{ $products->total() }} {{ __('messages.items') }}
                        </p>
                        
                        <div class="flex items-center gap-4 w-full sm:w-auto">
                            <label class="hidden sm:block text-[10px] font-black text-slate-300 uppercase tracking-widest">{{ __('messages.sort_by') }} :</label>
                    ...
                    @if($products->isEmpty())
                        <div class="bg-[#FCFCFC] rounded-[40px] p-24 text-center border border-slate-100 mt-12">
                             ...
                             <h2 class="text-2xl font-black text-slate-900 mb-2">{{ __('messages.no_results') }}</h2>
                             <p class="text-slate-400 font-bold uppercase tracking-widest text-[10px]">{{ __('messages.no_results') }}</p>
                             <a href="{{ route('shop.index') }}" class="mt-10 inline-block text-[10px] font-black text-primary uppercase tracking-[0.3em] hover:underline">{{ __('messages.reset_all') }}</a>
                        </div>
                    @else
    ...
    <div x-show="showFilters" class="fixed inset-0 z-[1000] md:hidden" style="display: none;">
        ...
        <div x-show="showFilters" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0" class="absolute inset-x-0 bottom-0 max-h-[90vh] bg-white rounded-t-[48px] shadow-2xl p-8 flex flex-col">
            ...
            <div class="flex justify-between items-center mb-10">
                <h3 class="text-2xl font-black tracking-tighter">{{ __('messages.filters_and_sorting') }}</h3>
            ...
            <div class="flex-1 overflow-y-auto space-y-10 custom-scrollbar pr-2">
                <!-- Sorting (Mobile) -->
                <div class="space-y-4">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-4 border-l-4 border-primary">{{ __('messages.sort_by') }}</p>
                    <div class="grid grid-cols-1 gap-2">
                         <a href="{{ request()->fullUrlWithQuery(['sort' => 'latest']) }}" class="flex items-center justify-between p-4 rounded-2xl {{ request('sort') == 'latest' || !request('sort') ? 'bg-primary text-white' : 'bg-slate-50 text-slate-600' }} font-bold text-xs uppercase tracking-widest transition-all">
                             {{ __('messages.newest') }} @if(request('sort') == 'latest' || !request('sort')) <i class="fas fa-check"></i> @endif
                         </a>
                         <a href="{{ request()->fullUrlWithQuery(['sort' => 'price_asc']) }}" class="flex items-center justify-between p-4 rounded-2xl {{ request('sort') == 'price_asc' ? 'bg-primary text-white' : 'bg-slate-50 text-slate-600' }} font-bold text-xs uppercase tracking-widest transition-all">
                             {{ __('messages.price_asc_label') }} @if(request('sort') == 'price_asc') <i class="fas fa-check"></i> @endif
                         </a>
                         <a href="{{ request()->fullUrlWithQuery(['sort' => 'price_desc']) }}" class="flex items-center justify-between p-4 rounded-2xl {{ request('sort') == 'price_desc' ? 'bg-primary text-white' : 'bg-slate-50 text-slate-600' }} font-bold text-xs uppercase tracking-widest transition-all">
                             {{ __('messages.price_desc_label') }} @if(request('sort') == 'price_desc') <i class="fas fa-check"></i> @endif
                         </a>
                    </div>
                </div>
                ...
                <!-- Categories (Mobile) -->
                <div class="space-y-4">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-4 border-l-4 border-primary">{{ __('messages.categories_label') }}</p>
                    <div class="flex flex-wrap gap-2">
                         <a href="{{ route('shop.index') }}" class="px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all {{ !request('category') ? 'bg-primary text-white' : 'bg-slate-50 text-slate-500 border border-slate-100' }}">{{ __('messages.view_all') }}</a>
                         @foreach($categories as $cat)
                ...
                <div class="space-y-6 pt-4">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-4 border-l-4 border-primary">{{ __('messages.price_range') }}</p>
                    <form action="{{ route('shop.index') }}" method="GET" class="space-y-6">
                         @if(request('category')) <input type="hidden" name="category" value="{{ request('category') }}"> @endif
