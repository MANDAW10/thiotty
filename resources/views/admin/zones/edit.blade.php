<x-admin-layout>
    <div class="mb-12">
        ...
        <h1 class="text-3xl font-black text-slate-900">Modifier la Zone</h1>
    </div>

    <div class="max-w-2xl bg-white p-8 md:p-12 rounded-[40px] border border-slate-100 shadow-sm">
        <form action="{{ route('admin.zones.update', $zone) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1">
                ...
                <input type="number" name="fee" value="{{ old('fee', $zone->fee) }}" required
                       class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-2 focus:ring-primary/20 transition-all">
