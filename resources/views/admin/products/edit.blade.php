    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-8">
            <div class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
                ...
                <div class="space-y-1" x-data="{ photoName: null, photoPreview: '{{ $product->image_url }}' }">
                            ...
                            <div class="w-full h-48 rounded-3xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-primary/50 group-hover:bg-primary/5">
                                <template x-if="!photoPreview">
                                    <div class="text-center">
                                        ...
                                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Cliquer pour changer</p>
                                    </div>
                                </template>
                                <template x-if="photoPreview">
